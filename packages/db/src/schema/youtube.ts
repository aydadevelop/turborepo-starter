...
export const ytFeedSourceModeValues = [
	"search",
	"game_channel",
	"user_channel",
	"user_channel_query",
...
export type YtFeedSourceMode = (typeof ytFeedSourceModeValues)[number];
...
// ─── Uploader Channel ────────────────────────────────────────────────────────
// A YouTube content creator / uploader channel (the person who made the video).
// Populated from discovery candidate data; enriched lazily.

export const ytUploaderChannel = sqliteTable(
	"yt_uploader_channel",
	{
		/** YouTube channel ID (UC…) — primary key, stable identifier */
		id: text("id").primaryKey(),
		/** Channel display name */
		name: text("name").notNull(),
		/** YouTube handle, e.g. @ZackScottGames */
		handle: text("handle"),
		thumbnailUrl: text("thumbnail_url"),
		subscriberCount: integer("subscriber_count"),
		videoCount: integer("video_count"),
		/** When channel metadata was last fetched from YouTube */
		metaFetchedAt: integer("meta_fetched_at", { mode: "timestamp_ms" }),
		...timestamps,
	}
);

// ─── Game Channel ─────────────────────────────────────────────────────────────
// YouTube's auto-generated game metadata page (UC… channel tied to a game title).
// Distinct from creator channels: no subscribers, no upload count — just game info.
// Populated by getGameChannel() scraping the watch page videoAttributeViewModel.

export const ytGameChannel = sqliteTable(
	"yt_game_channel",
	{
		/** YouTube channel ID (UC…) auto-generated by YouTube for this game */
		id: text("id").primaryKey(),
		/** Game display title as shown on YouTube */
		title: text("title").notNull(),
		thumbnailUrl: text("thumbnail_url"),
		/** When this game channel metadata was last fetched */
		metaFetchedAt: integer("meta_fetched_at", { mode: "timestamp_ms" }),
		...timestamps,
	}
);
...
export const ytFeed = sqliteTable(
	"yt_feed",
	{
		...
		/** Explicit feed source mode discriminator. */
		sourceMode: text("source_mode", { enum: ytFeedSourceModeValues })
			.notNull()
			.default("search"),
		searchQuery: text("search_query").notNull(),
		/**
		 * Optional YouTube channel ID (UC…) that scopes this feed's discovery.
		 * When set, discovery browses or searches this channel instead of doing a
		 * global keyword search. References yt_uploader_channel — the channel
		 * whose uploads we crawl (a creator OR a game publisher's upload channel).
		 */
		scopeChannelId: text("scope_channel_id").references(
			() => ytUploaderChannel.id
		),
		/**
		 * Optional YouTube playlist ID (PLxxx…) to sync videos from.
		 * When set, discovery fetches all videos in the playlist.
		 * Takes priority over scopeChannelId if both are set.
		 */
		playlistId: text("playlist_id"),
		/** Comma-separated query exclusions for sourceMode=search. */
		stopWords: text("stop_words"),
		/** Comma-separated title exclusions applied after discovery for all modes. */
		titleStopWords: text("title_stop_words"),
		...
		enableAsr: integer("enable_asr", { mode: "boolean" })
			...
			.default(false),
		/**
		 * Minimum video duration in seconds. Videos shorter than this are skipped
		 * during discovery — useful to exclude trailers and YouTube Shorts.
		 * null = no minimum (all durations accepted).
		 */
...
export const ytVideo = sqliteTable(
	"yt_video",
	{
		...
		title: text("title").notNull(),
		/**
		 * Uploader's YouTube channel ID (UC…). References yt_uploader_channel.
		 * Populated from discovery candidate data.
		 */
		uploaderChannelId: text("uploader_channel_id").references(
			() => ytUploaderChannel.id
		),
		/**
		 * YouTube's auto-generated game channel ID (UC…). References yt_game_channel.
		 * Populated by the getGameChannel() watch-page scraper during ingest,
		 * NOT set from the feed's scopeChannelId (those are uploader channels).
		 */
		gameChannelId: text("game_channel_id").references(() => ytGameChannel.id),
...
export const ytCluster = sqliteTable(
	"yt_cluster",
	{
		...
		state: text("state", { enum: ytClusterStateValues })
			...
			/** Deterministic signature for stable issue grouping. */
		...
		(table) => [
			...
			uniqueIndex("yt_cluster_org_fingerprint_unique").on(
