...
export async function publishListing(
	...
	if (!existingListing) throw new Error("NOT_FOUND");
	...
	if (!updatedListing) throw new Error("Update failed");
	...
	if (existingPub) {
		const [updated] = await db
			...
			.returning();
		if (!updated) throw new Error("Publication update failed");
		pub = updated;
	} else {
		const [inserted] = await db
			...
			.returning();
		if (!inserted) throw new Error("Publication insert failed");
		pub = inserted;
	}
