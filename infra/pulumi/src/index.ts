...
const apps = new DokkuApps(
	"apps",
	{
		...
		env: {
			...
			EMAIL_REPLY_TO: appConfig.get("emailReplyTo") ?? "",
			SMTP_HOST: appConfig.get("smtpHost") ?? "smtp",
			SMTP_PORT: appConfig.get("smtpPort") ?? "25",
			SMTP_SECURE: appConfig.get("smtpSecure") ?? "0",
			SMTP_IGNORE_TLS: appConfig.get("smtpIgnoreTls") ?? "1",
