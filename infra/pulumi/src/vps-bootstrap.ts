...
export class VpsBootstrap extends pulumi.ComponentResource {
	...
	constructor(
		...
		const installKey = new command.remote.Command(
			`${name}-install-key`,
			{
				connection: {
...
`,
		...
		const diskExpand = new command.remote.Command(
			`${name}-disk-expand`,
			{
				connection: conn,
				create: `
      ...
      `,
		...
		// ── Docker ────────────────────────────────────────────────────────────
		const docker = new command.remote.Command(
			`${name}-docker`,
			{
				connection: conn,
				create: `
      ...
      `,
		...
		// ── Dokku ─────────────────────────────────────────────────────────────
		const dokku = new command.remote.Command(
			`${name}-dokku`,
			{
				connection: conn,
				create: `
      ...
      `,
		...
		// ── Dokku plugins ─────────────────────────────────────────────────────
		const plugins = new command.remote.Command(
			`${name}-dokku-plugins`,
			{
				connection: conn,
				create: `
      ...
      `,
		...
		const sslhSetup = new command.remote.Command(
			`${name}-sslh`,
			{
				connection: conn,
				create: `
