...
const oneGbVpsProvider: pulumi.dynamic.ResourceProvider = {
	async create(inputs: OneGbVpsInputs): Promise<pulumi.dynamic.CreateResult> {
		...
		let ip = "";
		for (let i = 0; i < 120; i++) {
			const statusResp = await apiGet("/vds/list", {
				_token_: token,
				_key_: String(serverId),
			});
			const pending = statusResp?.[0]?.pending_creation ?? "1";

			if (String(pending) === "0") {
				const ipResp = await apiGet("/vds/ip/list", {
					_token_: token,
					_key_: String(serverId),
				});
				ip = ipResp?.[0]?.ip ?? "";
		...
		if (!ip) {
			throw new Error(`1gb: VPS ${serverId} never got an IP after 10min`);
		}

		// Fetch root password from VPS details
		const detailResp = await apiGet("/vds/list", {
			_token_: token,
			_key_: String(serverId),
		});
