...
<Card.Root class="sticky top-6">
	<Card.Header>
		...
		<Card.Description>
			Check live availability, preview the price, and send your request for
			{listingName}.
		</Card.Description>
	</Card.Header>
	<Card.Content class="space-y-4">
		...
		<div class="space-y-2">
			...
			<Textarea
				...
				bind:value={notes}
				rows={3}
				placeholder="Share any trip details or questions for the operator."
			/>
		</div>
		...
		<div class="space-y-2">
			...
			<Textarea
				...
				rows={3}
				placeholder="Accessibility, catering, or timing requests."
			/>
		</div>
				...
				{:else if quoteQuery.data}
					<div class="space-y-1 text-sm">
						<div class="flex items-center justify-between">
							...
							<span
								>{formatMoney(quoteQuery.data.baseCents, quoteQuery.data.currency)}</span
							>
						</div>
						<div class="flex items-center justify-between">
							<span class="text-muted-foreground">Adjustments</span>
							<span
								>{formatMoney(quoteQuery.data.adjustmentCents, quoteQuery.data.currency)}</span
							>
						</div>
						<div class="flex items-center justify-between">
							<span class="text-muted-foreground">Fees + taxes</span>
							<span
								>{formatMoney(quoteQuery.data.serviceFeeCents + quoteQuery.data.taxCents, quoteQuery.data.currency)}</span
							>
						</div>
						<div
						...
						>
							<span>Total</span>
							<span
								>{formatMoney(quoteQuery.data.totalCents, quoteQuery.data.currency)}</span
							>
						</div>
					</div>
				{/if}
		...
		{#if !isAuthenticated}
			<p class="text-xs text-muted-foreground">
				You can preview pricing and availability now. Signing in is only
				required when you send the final booking request.
			</p>
		{/if}
	</Card.Content>
	<Card.Footer class="flex flex-col items-stretch gap-3">
		<Button
			...
			disabled={!bookingSlotInput || availabilityKnown === false || quoteQuery.isPending || slotQuery.isPending || createBookingMutation.isPending}
		>
		...
		{#if bookingSlotInput && availabilityKnown === true && quoteQuery.data && !canSubmitBooking && isAuthenticated}
			<p class="text-xs text-muted-foreground">
				Pricing is ready, but the request can’t be submitted until the preview
				fully settles.
			</p>
		{/if}
	</Card.Footer>
