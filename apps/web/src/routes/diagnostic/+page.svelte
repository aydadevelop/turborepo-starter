	...
	const pages = [
		{
			...
			suspects: ["Duplicate $derived parsing", "New Date objects breaking reference equality", "$effect → store → query chain"],
		},
		{
			...
			suspects: ["refetchInterval triggers re-render of parent with large list", "State update on main thread during scroll"],
		},
		{
			...
			suspects: ["consumeEventIterator events → notifications $state update → header re-render", "Multiple authClient.useSession() subscriptions"],
		},
		{
			...
			suspects: ["$derived([...]) creates new array reference every cycle", "derived(sessionQuery, ...) creates new queryFn closures", "authClient.useSession() called in N components simultaneously"],
		},
	] as const;
</script>
...
<p class="mb-6 text-sm text-muted-foreground">
	Pages that isolate the suspected root causes of scroll jank. Open DevTools Performance tab, start
	recording, then scroll while a diagnostic page is open.
</p>
