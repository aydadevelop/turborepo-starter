<script lang="ts">
	...
	const shiftRequestsNeedingReview = $derived.by(() => {
		return myShiftRequests.filter((request) => {
			return (
				request.status === "pending" && request.customerDecision === "pending"
			);
		});
	});
	...
	const getExpiresAt = (
		...
		if (
			...
			bookingItem.status !== "awaiting_payment"
		) {
			return null;
		}
		if (
		...
		return new Date(
			toDate(bookingItem.createdAt).getTime() + BOOKING_EXPIRY_MS
		);
	};
	...
	<div class="mx-auto w-full max-w-6xl space-y-6 px-6 py-10">
		<div class="space-y-2">
			...
			<p class="text-sm text-muted-foreground">
				Personal booking history with statuses and cancellation request options.
			</p>
		</div>
		...
		<Card>
			<CardHeader>
				<CardTitle class="text-base">Affiliate Referrals (Redacted)</CardTitle>
				<CardDescription>
								...
								<div class="flex flex-wrap items-start justify-between gap-3">
									...
									<div class="flex flex-col items-end gap-2">
										...
										{#if expiresAt}
											...
											<span
												class={`text-xs font-medium ${countdown === "Expired" ? "text-destructive" : "text-amber-600"}`}
											>
