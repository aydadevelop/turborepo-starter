<script lang="ts">
	...
	const handleSubmit = async () => {
		if (!hasAuthenticatedSession($sessionQuery.data)) {
			errorMessage = "You need to sign in with an email account before creating an organization.";
			return;
		}
		...
		try {
			...
			const { data: createdOrganization, error } = await authClient.organization.create({
				name: trimmedName,
				slug: trimmedName
					.toLowerCase()
			...
			const createdOrganizationId = (createdOrganization as { id?: string } | null)?.id ?? null;
			...
			await Promise.all([
				...
				queryClient.invalidateQueries({ queryKey: orpc.canManageOrganization.key() }),
			]);

			const orgsResult = await authClient.organization.list();
			queryClient.setQueryData(queryKeys.organizations.all, orgsResult.data ?? []);

			goto(postCreateRedirectPath, { replaceState: true });
		} catch (error) {
			errorMessage = toFriendlyOrgCreateError(
				extractErrorMessage(error, "An error occurred while creating the organization.")
			);
			pending = false;
		}
	};
...
<div class="mx-auto max-w-2xl px-6 py-10 space-y-6">
	...
	{#if hasExistingOrg}
		<div class="flex items-center justify-between rounded-lg border bg-muted/40 px-4 py-3 text-sm">
			<span class="text-muted-foreground">You already have an organization.</span>
			<Button variant="ghost" size="sm" onclick={() => goto(postCreateRedirectPath)}>
				Go to Settings
			</Button>
		</div>
	{/if}
		...
		{#snippet children()}
			<div class="space-y-4">
				...
				>
					...
					<p class="mb-3">
						This Agreement governs the relationship between the User and the Service Provider in connection with the provision of workspace management services.
					</p>
					<p class="mb-3">
						The platform facilitates organization-level collaboration and workflow management on behalf of registered organizations.
					</p>
					<p class="mb-3">
						By creating an organization, the User agrees to the terms set forth in this Agreement and the platform's Privacy Policy.
					</p>
					<p class="mb-4 font-semibold">1. Acceptance</p>
					<p class="mb-3">1.1. Before creating an organization, the User must carefully read the terms of this Agreement. The User may not proceed if they do not agree to these terms.</p>
					<p class="mb-3">1.2. This Agreement is considered accepted at the moment the User submits the organization creation form.</p>
					<p class="mb-4 font-semibold">2. Subject Matter</p>
					<p class="mb-3">2.1. The Service Provider grants the User access to tools for managing tasks, reminders, notifications, and related operational workflows within the platform.</p>
					<p class="mb-4 font-semibold">3. Financial Terms</p>
					<p class="mb-3">3.1. Subscription pricing is listed on the Pricing page.</p>
					<p class="mb-3">3.2. Payment is required in full before access is granted.</p>
					<p class="mb-4 font-semibold">4. Cancellation and Refunds</p>
					<p class="mb-3">4.1. The User may cancel their subscription at any time. Access continues until the end of the current billing period.</p>
					<p class="mb-3">4.2. Refunds are handled on a case-by-case basis within 10 business days of the request.</p>
					<p class="mb-4 font-semibold">5. Liability</p>
					<p class="mb-3">5.1. Each party is responsible for fulfilling their obligations under this Agreement in accordance with applicable law.</p>
					<p class="mb-4 font-semibold">6. Personal Data</p>
					<p class="mb-3">6.1. Personal data is processed in accordance with the Privacy Policy and applicable data protection regulations.</p>
					<p class="mb-4 font-semibold">7. Governing Law</p>
					<p class="mb-3">7.1. Disputes shall first be resolved through negotiation. If no agreement is reached, disputes will be submitted to the competent court of jurisdiction.</p>
					<p class="mt-6 text-xs text-muted-foreground">
						Full agreement text available upon request. Document version: 2026-02-14.
					</p>
				</div>
				...
				<label class="flex cursor-pointer select-none items-start gap-3">
					...
					<span class="text-sm">
						{#if !scrolledToBottom}
							<span class="text-muted-foreground">Scroll to the bottom to accept the terms</span>
						{:else}
