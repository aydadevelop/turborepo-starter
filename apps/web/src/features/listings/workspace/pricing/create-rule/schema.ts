...
function parseRuleConditionJson(value: string) {
	try {
		...
		if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
			return { ok: false as const, message: "Condition JSON must be a JSON object." };
		}
		...
		return { ok: false as const, message: "Condition JSON must be valid JSON." };
	}
}
