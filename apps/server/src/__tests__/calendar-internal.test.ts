...
const retryFailedGoogleSyncsMock = vi.fn();
...
vi.mock("@my-app/calendar", () => ({
	...
	retryFailedGoogleSyncs: retryFailedGoogleSyncsMock,
}));
...
describe("calendarInternalRoutes", () => {
	beforeEach(() => {
		...
		retryFailedGoogleSyncsMock.mockReset();
		...
		retryFailedGoogleSyncsMock.mockResolvedValue({
			kind: "ok",
			provider: "google",
	...
	it("returns 404 when calendar sync token is not configured", async () => {
		const { calendarInternalRoutes } = await import("../routes/calendar-internal");
