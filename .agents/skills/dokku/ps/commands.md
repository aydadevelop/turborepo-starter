...
Rollback an app to the previous deploy.

```bash
dokku ps:rollback <app>
dokku ps:rollback --all
```

Notes:

- Rollback restores the previous release image.
- Environment/config changes made via `dokku config:set` are **not** automatically reverted.
- Verify after rollback with `dokku ps:report <app> --deployed` and app health checks.
