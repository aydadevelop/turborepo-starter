...
## Pulumi-first boundary (repo-specific)

This repository treats **Pulumi as the source of truth** for persistent infrastructure changes.

- Use **Pulumi** (`infra/pulumi/`, then `pulumi up`) for durable changes: apps, domains, proxy/network shape, plugins, and baseline config.
- Use **manual Dokku CLI** for operational actions: logs, one-off jobs, temporary debugging, manual rollback (`dokku ps:rollback`), and emergency inspection.
- If a manual Dokku action becomes long-lived policy, codify it back in Pulumi so the state remains reproducible.
...
| Process (ps)               | [ps/commands.md](ps/commands.md)           | scale, rebuild, restart, start, stop, rollback               |
...
- **Rollback:** `dokku ps:rollback <app>`
