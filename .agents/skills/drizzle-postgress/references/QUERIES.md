...
## Repository conventions (reads first)

- Keep transport thin. Query code belongs in the owning domain/service package or in `packages/db` test helpers.
- Default to explicit filters, explicit ordering, and deterministic result shapes.
- Do not assume any cache exists. In beta16, cache is opt-in and currently absent from this repo's DB wiring.
- Treat JavaScript `Date` parameters as UTC instants.
- When returning data across a boundary, select only the columns you need instead of shipping full rows out of habit.

### Query checklist

1. Is the sort order deterministic?
2. Are the filter columns indexed?
3. Does the query return only the fields the caller needs?
4. Is pagination stable under concurrent writes?
5. Are you accidentally encoding local-time semantics into `Date` math?
...
### Repo-friendly dynamic filters

- Prefer `and(...maybeUndefinedConditions)` over string-building SQL.
- If the query will be reused by multiple callers, wrap it in a small query function instead of duplicating inline conditions.
- For optional search terms, keep the raw `sql` fallback local to the query helper so the rest of the call sites stay typed.
...
Offset pagination is acceptable for admin/operator views and small datasets. For user-facing or growing tables, prefer cursor-based pagination with a stable sort key.
...
For timestamp-backed cursors in this repo, pair the timestamp with a tiebreaker column such as `id` to avoid duplicate/missing rows when multiple records share the same instant.
...
### Join guidance for this repo

- Index every foreign key used in `JOIN` and `WHERE` clauses.
- Prefer explicit joins for cross-domain read models where the response shape is curated.
- Use relational queries only when the relationship graph is stable and the query shape is truly nested.
- If you need computed or JSON expressions, keep them near the query boundary with `sql```, not scattered through calling code.
...
### Beta16 cache reminder

Even if cache is configured later, Drizzle cache does not currently cover all query forms equally — especially relational queries and transactions. Design correctness first; treat caching as an explicit optimization layer.
