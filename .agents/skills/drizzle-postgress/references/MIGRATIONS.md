...
## Repository conventions (beta16)

- Runtime schema: `packages/db/src/schema/index.ts`
- Migration output: `packages/db/src/migrations`
- Dev config: `packages/db/drizzle.config.dev.ts`
- Default config: `packages/db/drizzle.config.ts`
- Real-Postgres verification lane: `packages/db/scripts/verify-postgres-baseline.mjs`

### Canonical workflow in this repo

1. Edit schema files in `packages/db/src/schema/**`
2. Generate SQL with `drizzle-kit generate`
3. Review generated SQL before applying it
4. Apply locally with `drizzle-kit migrate`
5. If the change matters for reproducibility or DDL correctness, run the real Postgres verification lane

### Important policy

- `generate` + `migrate` is the default, reviewable path.
- `push` is for scratch/prototyping only and is **not** the reproducibility path for this repo.
- `--ignore-conflicts` exists in beta16, but do not normalize on it. Only use it after manual analysis of why the migration state diverged.
...
In this repo, keep `push` out of baseline/bootstrap workflows and committed deployment paths.
...
### Repo verification add-on

For changes that affect DDL correctness, seeding, or reset/restore behavior, run the dedicated verification script after local migration testing. The repo already includes a lane that resets a real Postgres database, migrates, seeds, exports, re-imports, and validates row counts.
...
For this repo, review the files under `packages/db/src/migrations/**` and confirm:

- no accidental table drops/renames
- no missing foreign-key indexes
- timestamp/date column types still match the intended semantics
- enum changes are additive or intentionally breaking
...
```
  ...
  ### Beta16 notes worth remembering

  - `drizzle-kit` beta rewrites improved diffing, but generated SQL still deserves human review.
  - Migration conflicts are easier to recover from than silent semantic drift — do not paper over unexpected diffs with forceful flags.
