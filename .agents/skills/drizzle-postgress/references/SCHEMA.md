...
- This workspace is pinned to `drizzle-orm@1.0.0-beta.16-*`.
- Shared schema lives in `packages/db/src/schema`; the entrypoint is `packages/db/src/schema/index.ts`.
- Common audit columns come from `packages/db/src/schema/columns.ts`.
- New schema-validation integrations should prefer `drizzle-orm/typebox`, `drizzle-orm/zod`, etc. instead of legacy standalone `drizzle-*` packages.

### Time semantics: the rule that saves pain

Use the column type that matches the business meaning — not just the variable name.

- **Absolute instant** → `timestamp(..., { withTimezone: true, mode: 'date' })`
  - Good for `createdAt`, `updatedAt`, `processedAt`, `startsAt`, `endsAt`, `expiresAt`
- **Calendar-only day / local business date** → `date(..., { mode: 'string' })`
  - Good for a holiday, booking day, closure day, season start day, billing date

`mode: 'date'` is still supported in beta16; it is **not deprecated**. The real risk is using a JavaScript `Date` when the business concept is “a day on a calendar” instead of “a precise instant in UTC”.

### Repo audit note

`packages/db/src/schema/availability.ts` contains `listingAvailabilityException.date` as a timestamp-like field even though the table comment describes one-off date overrides. Treat fields like this as audit candidates before copying the pattern elsewhere.
...
```typescript
// Absolute instant stored and read as JS Date
startsAt: timestamp('starts_at', { withTimezone: true, mode: 'date' }).notNull(),

// Calendar-only value, safer as string
businessDate: date('business_date', { mode: 'string' }).notNull(),
```

- Prefer `withTimezone: true` for persisted instants.
- Use UTC in tests and fixtures.
- Avoid using `date(..., { mode: 'date' })` unless you explicitly want a JavaScript `Date` object and have thought through timezone normalization.
...
The live repo helper uses explicit SQL defaults:
...
This is valid in beta16. `defaultNow()` is also valid on concrete timestamp builders, but keep the repo style consistent unless there is a good reason to change it.

### Builder-specific beta note

In the beta line, methods such as `.defaultNow()`, `.generatedAlwaysAsIdentity()`, and `.generatedByDefaultAsIdentity()` are attached to the concrete builders that support them. Do not design helper abstractions that assume every builder exposes the same methods.
